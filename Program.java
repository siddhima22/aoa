...
public class Bankersalgo2 {
    ...
    public boolean resourceRequest(int process, int[] request) {
        boolean safe = true;
        for (int i = 0; i < m; i++) {
        ...
        if (!safe) {
            System.out.println("Request exceeds maximum claim for the resource. Process must wait.");
            return false;
        }
        ...
        if (!safe) {
            System.out.println("Process must wait for the resource since it is not available for use.");
            return false;
        }
    ...
    public static void main(String[] args) {
        Bankers2 bankers = new Bankers2();
        bankers.initializeValues();
        bankers.calculateNeed();
        bankers.isSafe();

        Scanner scanner = new Scanner(System.in);
        System.out.print("Enter the process number for resource request: ");
        int process = scanner.nextInt() - 1;

        System.out.print("Enter the resource request for process " + (process + 1) + ": ");
        int[] request = new int[bankers.m];
        for (int i = 0; i < bankers.m; i++) {
            request[i] = scanner.nextInt();
        }

        if (bankers.resourceRequest(process, request)) {
            System.out.println("Request granted. System is still safe.");
        }
